...
var createFighterButtons = [new Button('back', function() {return canvas.width/60;}, function() {return canvas.height/33.75;}, function() {return canvas.width/10;}, function() {return canvas.height/15;}, 3, function() {demo = false; fighterSelect = false; createFighter=false;}, function() {return true;}, function() {return 'white';}, function() {return ['Back'];}, 'black', function() {return (canvas.width/70).toString() + 'px Arial';}, function() {return null;}, function() {return true;}),
  ...
  new Button('newProjectile', function() {return 0.40625*canvas.width-this.getWidth()*2;}, function() {return canvas.height/4 + newFighterProjectiles*canvas.height/24;}, function() {return 0.09375*canvas.width;}, function() {return canvas.height/24;}, 3, function() {createFighterButtonsLength+=1; newFighterProjectiles+=1; newFighterProjectileNum+=1; newProjectileButton(); newFighterData['projectiles']['Projectile' + newFighterProjectileNum.toString()] = {'x': 0, 'y': 0, 'width': 0.0166667, 'height': 0.05, 'velX': 0.01, 'velY': -0.01, 'weight': 1, 'hitsLeft': 1};}, function() {return true;}, function() {return ((newFighterAction == this.id) ? 'white' : 'rgba(210, 210, 210, 1)');}, function() {return ['New Projectile'];}, 'black', function() {return (canvas.width/85).toString() + 'px Arial';}, function() {return null;}, function() {return true;}),
  new Button('deleteProjectile', function() {return 0.685*canvas.width;}, function() {return 0.865*canvas.height;}, function() {return canvas.width/10;}, function() {return canvas.height/30;}, 3, function() {if (Object.keys(newFighterData['projectiles']).length > 1) {var found; var marker = false; for (var i in newFighterData['projectiles']) {if (marker) {found = i; break;} if (i == newFighterAction) {marker = true;} else {found = i;}} delete newFighterData['projectiles'][newFighterAction]; delete newFighterData['hitboxes'][newFighterAction]; delete newFighterData['attacks'][newFighterAction]; newFighterProjectiles -= 1; for (var i in imgs['new']) {delete imgs['new'][i][newFighterAction];} var prevAction = newFighterAction; var projNum = 0; var hit = false; for (var i in createFighterButtons) {if (createFighterButtons[i].id == prevAction) {delete createFighterButtons[i]; hit = true;} else {var gotcha = false; for (var j in newFighterData['projectiles']) {if (j == createFighterButtons[i].id) {gotcha = true; break;}} if (gotcha) {if (hit) {let num = projNum; createFighterButtons[i]['getY'] = function() {return canvas.height/4 + num*canvas.height/24;};} projNum += 1;} if (createFighterButtons[i].id == found) {createFighterButtons[i].onClick();}}}} else {delete newFighterData['projectiles'][newFighterAction]; delete newFighterData['hitboxes'][newFighterAction]; delete newFighterData['attacks'][newFighterAction]; newFighterProjectiles -= 1; for (var i in imgs['new']) {delete imgs['new'][i][newFighterAction];} var prevAction = newFighterAction; for (var i in createFighterButtons) {if (createFighterButtons[i].id == prevAction) {delete createFighterButtons[i];} else if (createFighterButtons[i].id == 'idle') {createFighterButtons[i].onClick();}}}}, function() {return true;}, function() {return 'white';}, function() {return ['Delete Projectile'];}, 'black', function() {return (canvas.width/85).toString() + 'px Arial';}, function() {return null;}, function() {return (newFighterAction != 'stock' && !newFighterData['effects'][newFighterAction]);}),
      ...
      for(var l in newFighterData['attacks'][proj]) {
        if (newFighterData['attacks'][proj][l].frames[j]) {
          for (var n in newFighterData['attacks'][proj][l].frames[j]) {
            console.log('Attack: ' + newFighterData['attacks'][proj][l].frames[j][n]);
            console.log('Hitbox: ' + j);
            if (!tempAttack && newFighterData['attacks'][proj][l].frames[j][n] == j) {
              tempAttack = l;
              console.log('Found!' + tempAttack);
              break;
            }
          }
        }
...
function render() {
      ...
      for (var drawPlayer in ((game.started) ? game.players : [player])) {
        ...
        for (var i in tempPlayer.projectiles) {
          ...
          if (projectile.facing == 'right') {
            ...
            context.translate(canvas.width, 0);
            context.scale(-1, 1);
            context.drawImage(tempSheet, drawProjectileFrame*(tempSheet.width/data.frames), 0, tempSheet.width/data.frames, tempSheet.height, canvas.width - projectile.x*canvas.width - data.width, projectile.y*canvas.height, data.width*canvas.width, data.height*canvas.height);
            context.setTransform(1, 0, 0, 1, 0, 0);
          }
        }
  ...
  if(game && game != null && !game.started) {
    ...
    if (imgs['new'] && imgs['new'][newFighterSprite] && imgs['new'][newFighterSprite][newFighterAction] && imgs['new'][newFighterSprite][newFighterAction][newFighterFrame]) {
      context.drawImage(imgs['new'][newFighterSprite][newFighterAction][newFighterFrame], canvas.width/2 - (((newFighterAction == 'stock' || newFighterData['effects'][newFighterAction]) ? newFighterData['spriteWidth'] : newFighterData['projectiles'][newFighterAction]['width'])*canvas.width)/2, canvas.height/2 - (((newFighterAction == 'stock' || newFighterData['effects'][newFighterAction]) ? newFighterData['spriteHeight'] : newFighterData['projectiles'][newFighterAction]['height'])*canvas.height)/2, ((newFighterAction == 'stock' || newFighterData['effects'][newFighterAction]) ? newFighterData['spriteWidth'] : newFighterData['projectiles'][newFighterAction]['width'])*canvas.width, ((newFighterAction == 'stock' || newFighterData['effects'][newFighterAction]) ? newFighterData['spriteHeight'] : newFighterData['projectiles'][newFighterAction]['height'])*canvas.height);
    }
