			...
			for (var k=0; k<tempFrames.length; k++) {
				...
				for (var l=0; l<tempEffect.length; l++) {
					...
					if (tempArg[0] == 'projectile') {
						...
						effects[rawDataArray[i]][tempFrames[j]]['lingering'] = tempArg[1];
					} if (tempArg[0] == 'turnable') {
						effects[rawDataArray[i]][tempFrames[j]]['turnable'] = true;
...
function encodeNewFighter() {
	...
	imgs['demo'] = compileNewFighterImagesHorizontally();
...
function compileNewFighterImagesHorizontally(data) {
	...
	for(var i in data) {
		new_imgs[i] = {};
		for(var j in data[i]) {
			if(data[i][j] && Object.keys(data[i][j]).length > 0) {
				var tempCanvas = document.createElement('canvas');
				var tempContext = tempCanvas.getContext('2d');
				tempContext.imageSmoothingEnabled = false;
				tempCanvas.width = data[i][j][0].width*Object.keys(data[i][j]).length;
				tempCanvas.height = data[i][j][0].height;

				for(var k in data[i][j]) {
					if(data[i][j][k] != null) {
						tempContext.drawImage(data[i][j][k], 0, 0, data[i][j][k].width, data[i][j][k].height, k*(tempCanvas.width/Object.keys(data[i][j]).length), 0, tempCanvas.width/Object.keys(data[i][j]).length, tempCanvas.height);
					}
				}
